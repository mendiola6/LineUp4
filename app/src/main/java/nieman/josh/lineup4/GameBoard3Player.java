 ...
 */
public class GameBoard3Player extends View {
    ...
    private int winner;
    private int timesCheckedForWinner = 0;

    //some constructors
    public GameBoard3Player(Context context){
        this(context,null);
    }

    public GameBoard3Player(Context context, AttributeSet attr){
        super(context,attr);
        ...
        player3Color = Color.CYAN;
        player3 = new Paint();
        player3.setStyle(Paint.Style.FILL);
        player3.setColor(player3Color);
    ...
    public void setPlayer3Color(int color){
        player3Color = color;
        player3.setColor(player3Color);
    }

    public int getPlayer3Color(){ return player3Color; }
    ...
    public void setPlayer3Name(String name){  player3Name = name; }
            ...
            for(int c = 0; c < numColumns; c++){
                for(int r = 0; r < numRows; r++){
                   if(board[c][r] != 0) // so there is something in the box
                   {
                       ...
                       else if(board[c][r] == 3)//player3's color
                       {
                           canvas.drawCircle((float)(boxWidth *c+.5* boxWidth), (float)(boxHeight*r+.5*boxHeight),(float)radius,player3);
                       }
                       else{
        ...
        if(e.getAction() == MotionEvent.ACTION_DOWN && winner == 0){
            ...
            if(row >=0){
                if(playersTurn == 1){
                    ...
                    board[column][row] = 3; //player3's number
                }
            }
            ...
            if(timesCheckedForWinner == (numRows * numColumns)){
                winner = 5;
            }
            ...
            if(winner != 0){
                if(winner == 1){
                    ...
                    Toast toast = Toast.makeText(this.getContext(), player3Name + " won the game!", Toast.LENGTH_LONG);
                    toast.setGravity(Gravity.CENTER,0,0);
                    toast.show();
                    Log.d("FOUND_WINNER", player3Name + " won the game");
                }
                else if(winner == 5){
                    Toast toast = Toast.makeText(this.getContext(), "It's a tie!", Toast.LENGTH_LONG);
                    toast.setGravity(Gravity.CENTER,0,0);
                    toast.show();
                    Log.d("FOUND_WINNER", "It's a tie.");
                }
            }
            ...
            Toast toast = Toast.makeText(this.getContext(),"Game Over!", Toast.LENGTH_SHORT);
            toast.setGravity(Gravity.CENTER,0,0);
            toast.show();
        }
